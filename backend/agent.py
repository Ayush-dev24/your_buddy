...
class AIAgent:
    ...
    def _build_prompt(self, mode: str, query: str, context: str) -> str:
        mode = str(mode or "qa").strip().lower()
    ...
    def stream_answer(
        ...
        for model_name in self.model_candidates:
            try:
                ...
                if not response_text:
                    response_text = "AI returned an empty response. Try a different prompt or model."
                    yield response_text
                ...
                if self._is_auth_error_text(str(e)):
                    saw_auth_error = True
                logger.warning(
                    "Gemini stream failed (model=%s): %s",
                    model_name,
                    str(e),
                )
        ...
        if last_error:
            if saw_auth_error:
                ...
                message = f"Gemini generation failed: {str(last_error) or 'unknown error'}"
        ...
        message = "AI returned an empty response. Try a different prompt or model."
        self._firestore_save(
            "chat_logs",
            {
                "user_id": user_id,
